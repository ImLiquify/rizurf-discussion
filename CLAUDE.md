...
- **Group roles**: a group's `created_by` user is its **owner**; the owner can make members **sub-admins** (`group_members.role = 'admin'`, `PATCH /api/groups/:groupId/members/:userId`). Owner + sub-admins manage the group (rename/photo, add/remove members, sub-groups, delete any topic or message in it); only the owner assigns sub-admins, removes a sub-admin, or deletes the group. **Company admins/managers get no bypass inside groups** (`requireGroupRole`/`isGroupManager`/`canModerateMessage` in `server/rizurfApi.js`; frontend mirrors via `myRole` from `GET /api/groups?mine=1`). Anyone can create a group; everything is enforced server-side, not just hidden in the UI.
- **Chat message actions**: replies (`feedback.reply_to_id`, must be in the same topic), pins (`feedback.pinned_at`, shared, anyone in the chat can pin), stars (`message_stars`, private per user), copy, edit (own, within 1h), delete (`canModerateMessage`). Images/videos open in a zoomable lightbox; attachment GETs honour HTTP Range so videos can seek. Pinned messages rotate in a carousel under the chat title (`renderPinnedTicker`). Messages that @mention the viewer get a full-width highlighted line that pulses twice once per session (`chatMessageLineHtml`/`pulsedMentionIds`).
