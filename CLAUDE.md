...
- **Chat message actions**: replies (`feedback.reply_to_id`, must be in the same topic), pins (`feedback.pinned_at`, shared, anyone in the chat can pin), stars (`message_stars`, private per user), copy, edit (own, within 1h), delete (`canModerateMessage`). Images/videos open in a zoomable lightbox; attachment GETs honour HTTP Range so videos can seek.
