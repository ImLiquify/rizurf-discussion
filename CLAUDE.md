...
- **Stickers** are static images in `dist/assets/stickers/`, listed in the `STICKERS` array in `index.html` (`{ id, file, label }`), and sent as the plain message text `[sticker:<id>]` — no schema involved; `messagePreviewText()` turns that into "Sticker" for previews. Adding stickers = drop the files in that folder (force-add, `dist/` is gitignored) + add entries to `STICKERS`.
