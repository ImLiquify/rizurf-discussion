...
- **Group roles**: a group's `created_by` user is its **owner**; the owner can make members **sub-admins** (`group_members.role = 'admin'`, `PATCH /api/groups/:groupId/members/:userId`). Owner + sub-admins manage the group (rename/photo, add/remove members, sub-groups, delete any topic in it); only the owner assigns sub-admins, removes a sub-admin, or deletes the group. **Company admins/managers get no bypass inside groups** (`requireGroupRole`/`isGroupManager`/`canModerateMessage` in `server/rizurfApi.js`; frontend mirrors via `myRole` from `GET /api/groups?mine=1`). Anyone can create a group; everything is enforced server-side, not just hidden in the UI.
- **Chat message actions**: replies (`feedback.reply_to_id`, must be in the same topic), pins (`feedback.pinned_at`/`pinned_until`, shared, anyone in the chat can pin, always for a set time — 1h/1d/7d/30d, `PIN_DURATIONS` — and `listFeedback` reports an expired pin as unpinned), stars (`message_stars`, private per user), copy, edit (own, within 1h), delete — **chat messages can only be deleted (for everyone) by their author**; anyone can "delete for me" (`POST /api/feedback/:id/hide` → `message_hidden`, filtered out of `listFeedback` for that viewer). Both go through a confirm pop-up (`openDeleteConfirm`). Images/videos open in a zoomable lightbox; attachment GETs honour HTTP Range so videos can seek. Pinned messages rotate in a carousel under the chat title (`renderPinnedTicker`). Messages that @mention the viewer get a full-width highlighted line that pulses twice once per session (`chatMessageLineHtml`/`pulsedMentionIds`).
